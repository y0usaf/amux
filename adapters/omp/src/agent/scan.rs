...
mod tests {
    use super::*;
    use crate::agent::files::{OMP_AGENT_DIR_ENV, OMP_SESSION_DIR_ENV};
    ...
    impl EnvGuard {
        fn set_home(value: &Path) -> Self {
            ...
            std::env::remove_var(OMP_AGENT_DIR_ENV);
            std::env::remove_var(OMP_SESSION_DIR_ENV);
            Self { _lock: lock, old }
        }
    }

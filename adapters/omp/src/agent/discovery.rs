...
mod tests {
    ...
    #[test]
    fn omp_launch_display_formats_binary_and_runner_variants() {
        let binary = OmpLaunch::Binary(PathBuf::from("/tmp/omp"));
        assert_eq!(binary.display(), "/tmp/omp");

        let runner = OmpLaunch::PackageRunner {
            runner: PathBuf::from("/usr/bin/env"),
            prefix_args: vec!["bunx".into(), "omp".into()],
        };
        assert_eq!(runner.display(), "/usr/bin/env bunx omp");
    }

    #[test]
    fn which_finds_executable_from_path() {
        let dir = TestDir::new();
        let exe = dir.path().join("omp-test");
        write_executable(&exe, "#!/bin/sh\nexit 0\n");
        let _guard = EnvGuard::set("PATH", dir.path().as_os_str());

        assert_eq!(which("omp-test"), Some(exe));
    }

    #[test]
    fn try_binary_reports_version_for_existing_executable() {
        let dir = TestDir::new();
        let exe = dir.path().join("omp");
        write_executable(&exe, "#!/bin/sh\necho omp-test 1.2.3\n");

        let result = try_binary(&exe).unwrap();
        match result.launch {
            OmpLaunch::Binary(path) => assert_eq!(path, exe),
            OmpLaunch::PackageRunner { .. } => panic!("expected binary launch"),
        }
        assert_eq!(result.version.as_deref(), Some("omp-test 1.2.3"));
    }
    ...
    fn well_known_locations_include_user_and_system_defaults() {
        ...
        with_env_var_removed("OMP_HARNESS_SKIP_SYSTEM_DISCOVERY_PATHS", || {
            let locations = well_known_locations(home);

            assert!(locations.contains(&home.join(".bun/bin/omp")));
            assert!(locations.contains(&home.join(".volta/bin/omp")));
            assert!(locations.contains(&PathBuf::from("/usr/local/bin/omp")));
            assert!(locations.contains(&PathBuf::from("/run/current-system/sw/bin/omp")));
        });
    }
    ...
    fn config_override_bypasses_cached_default_discovery() {
        let dir = TestDir::new();
        let cached = dir.path().join("omp-cached");
        let override_path = dir.path().join("omp-override");
        write_executable(&cached, "#!/bin/sh\nexit 0\n");
        write_executable(&override_path, "#!/bin/sh\nexit 0\n");
        let _guard = EnvGuard::set("OMP_BINARY", cached.as_os_str());
        ...
        match result.launch {
            OmpLaunch::Binary(path) => assert_eq!(path, override_path),
            OmpLaunch::PackageRunner { .. } => panic!("expected binary launch"),
        }
        reset_discovery_cache();
    }
    ...
    fn invalid_config_override_does_not_fall_back_to_env_or_path() {
        let dir = TestDir::new();
        let path_pi = dir.path().join("omp");
        let missing = dir.path().join("missing-omp");
        write_executable(
            &path_pi,
            "#!/bin/sh
exit 0
",
        ...
        with_env_var_set("PATH", path.as_os_str(), || {
            with_env_var_removed("OMP_BINARY", || {
                reset_discovery_cache();
...
",
        ...
        with_env_var_set("PATH", path.as_os_str(), || {
            with_env_var_set("OMP_BINARY", missing.as_os_str(), || {
                reset_discovery_cache();
    ...
    fn default_discovery_caches_first_result_while_discover_fresh_tracks_env_changes() {
        let dir = TestDir::new();
        let first = dir.path().join("omp-a");
        let second = dir.path().join("omp-b");
        ...
        let (first_result, second_cached, second_fresh) = with_env_var_removed("PATH", || {
            with_env_var_removed("HOME", || {
                reset_discovery_cache();
                let first_result = with_env_var_set("OMP_BINARY", first.as_os_str(), || {
                    discover(None).expect("first discovery")
                });
                let second_cached = with_env_var_set("OMP_BINARY", second.as_os_str(), || {
                    discover(None).expect("cached discovery")
                });
                let second_fresh = with_env_var_set("OMP_BINARY", second.as_os_str(), || {
        ...
        match first_result.launch {
            OmpLaunch::Binary(path) => assert_eq!(path, first),
            OmpLaunch::PackageRunner { .. } => panic!("expected binary launch"),
        }
        ...
        match second_fresh.launch {
            OmpLaunch::Binary(path) => assert_eq!(path, second),
            OmpLaunch::PackageRunner { .. } => panic!("expected binary launch"),
        }
    }
    ...
    fn package_runner_falls_back_from_missing_bunx_to_working_npx() {
        ...
        match result.launch {
            OmpLaunch::PackageRunner {
                ...
                assert_eq!(prefix_args, vec!["@oh-my-pi/pi-coding-agent"]);
            }
            OmpLaunch::Binary(_) => panic!("expected package runner launch"),
        }
    }
    ...
    fn discover_finds_binary_on_path_after_cache_reset() {
        ...
        with_env_var_set("HOME", dir.path().as_os_str(), || {
            with_env_var_set("PATH", path.as_os_str(), || {
                with_env_var_removed("OMP_BINARY", || {
                    with_env_var_set(
                        "OMP_HARNESS_SKIP_SYSTEM_DISCOVERY_PATHS",
                        OsStr::new("1"),
                        || {
                            reset_discovery_cache();
                            assert!(discover(None).is_none());
                            ...
                            write_executable(&exe, "#!/bin/sh\nexit 0\n");
                            reset_discovery_cache();
                            let result = discover(None).expect("should discover omp on PATH");
                            match result.launch {
                                OmpLaunch::Binary(path) => assert_eq!(path, exe),
                                OmpLaunch::PackageRunner { .. } => panic!("expected binary launch"),
                            }

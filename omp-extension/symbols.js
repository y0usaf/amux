...
// (AGENT_HARNESS_OMP_ASCII / AGENT_HARNESS_SYMBOL_OVERRIDES), so the extension
...
export function resolveSymbols(env = process.env) {
	const base = env.AGENT_HARNESS_OMP_ASCII === "1" ? PRESETS.ascii : PRESETS.unicode
	...
	try {
		...
		console.warn(`omp-harness: ignoring invalid AGENT_HARNESS_SYMBOL_OVERRIDES: ${raw}`)
		return base
	}

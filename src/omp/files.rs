...
pub(crate) const OMP_AGENT_DIR_ENV: &str = "PI_CODING_AGENT_DIR";
pub(crate) const OMP_SESSION_DIR_ENV: &str = "PI_CODING_AGENT_SESSION_DIR";
pub(crate) const DEFAULT_AGENT_DIR_REL: &str = ".omp/agent";
pub(crate) const SESSIONS_DIR_NAME: &str = "sessions";
pub(crate) const ARCHIVE_DIR_NAME: &str = "ARCHIVE";

pub fn socket_path() -> PathBuf {
    app_runtime_dir().join(format!("omp-sidecar-{}.sock", std::process::id()))
}

pub fn is_omp_session_path(path: &Path) -> bool {
...
pub fn archive_session_file(source: &Path) -> Result<(), String> {
    let Some(dest_dir) = archive_dir() else {
        return Err(format!(
            "cannot archive session {}: omp archive dir unavailable",
            source.display()
        ));
    };
...
pub fn restore_session_file(source: &Path, project_path: &Path) -> Result<(), String> {
    let Some(dest_dir) = live_project_dir(project_path) else {
        return Err(format!(
            "cannot restore session {}: omp live dir unavailable for {}",
            source.display(),
...
fn agent_dir() -> Option<PathBuf> {
    env_path(OMP_AGENT_DIR_ENV)
        .or_else(|| {
            // Mirrors omp's DirResolver: once `$XDG_DATA_HOME/omp` exists it
            // becomes the data root, flattening sessions to `<root>/sessions`.
            let app_root = std::env::var_os("XDG_DATA_HOME").map(PathBuf::from)?.join("omp");
            app_root.exists().then_some(app_root)
        })
        .or_else(|| Some(home_dir()?.join(DEFAULT_AGENT_DIR_REL)))
}

fn configured_session_dir() -> Option<PathBuf> {
    env_path(OMP_SESSION_DIR_ENV)
}
...
mod tests {
    ...
    impl EnvGuard {
        ...
        fn set_paths(home: &Path, agent_dir: Option<&Path>, session_dir: Option<&Path>) -> Self {
            ...
            let keys = ["HOME", OMP_AGENT_DIR_ENV, OMP_SESSION_DIR_ENV];
            let old = keys
                .into_iter()
            ...
            match agent_dir {
                Some(value) => std::env::set_var(OMP_AGENT_DIR_ENV, value),
                None => std::env::remove_var(OMP_AGENT_DIR_ENV),
            }
            match session_dir {
                Some(value) => std::env::set_var(OMP_SESSION_DIR_ENV, value),
                None => std::env::remove_var(OMP_SESSION_DIR_ENV),
            }
    ...
    #[test]
    fn is_omp_session_path_matches_only_under_pi_root() {
        let home = TestDir::new();
        let _guard = EnvGuard::default_paths(home.path());
        let inside = home
            .path()
        ...
        assert!(is_omp_session_path(&inside));
        assert!(!is_omp_session_path(&outside));
    }

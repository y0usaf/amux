...
- **Canon applicability:** `[[canon:no-privileged-path]]` is n/a: the harness has no plugin story; only Pi's extension surface exists. Reverses when a second in-Pi companion ships. `[[canon:daemon-thin-client]]` is n/a: one viewer owns one lifetime and no state outlives it; Pi's session directory and `~/.pi` persist what matters. Reverses if sessions must stay warm without a viewer.

...
fn first_jsonl_value(reader: &mut impl BufRead) -> Option<Value> {
    ...
    loop {
        ...
        // omp >= 18 writes a fixed-width `{"type":"title",...}` slot as the
        // physical first line, before the `session` header (upstream loaders
        // strip it: session-loader.ts splitTitleSlot). Skip it so the header
        // check below sees the real `session` entry.
...
mod tests {
    use super::*;
    use crate::agent::implementation::store::{AGENT_DIR_ENV, SESSION_DIR_ENV};
    ...
    impl EnvGuard {
        fn set_home(value: &Path) -> Self {
            ...
            let keys = ["HOME", "XDG_STATE_HOME", AGENT_DIR_ENV, SESSION_DIR_ENV];
            let old = keys
                .into_iter()
    ...
    #[test]
    fn first_jsonl_value_skips_title_slot_line() {
        let mut input = std::io::Cursor::new(concat!(
            "{\"type\":\"title\",\"v\":1,\"title\":\"named\",\"updatedAt\":\"2024-01-01T00:00:00Z\",\"pad\":\"  \"}\n",
            "{\"type\":\"session\",\"id\":\"ok\"}\n",
        ));
        let value = first_jsonl_value(&mut input).unwrap();
        assert_eq!(value.get("type").and_then(Value::as_str), Some("session"));
    }

    #[test]
    fn session_meta_parses_title_slot_prefixed_session_file() {
        let dir = TestDir::new();
        let path = dir.path().join("session.jsonl");
        fs::write(
            &path,
            concat!(
                "{\"type\":\"title\",\"v\":1,\"title\":\"can we make an adapter\",\"updatedAt\":\"2024-01-01T00:00:00Z\",\"pad\":\"  \"}\n",
                "{\"type\":\"session\",\"id\":\"abc\",\"cwd\":\"/tmp/project\",\"timestamp\":\"2024-01-01T00:00:00Z\"}\n",
                "{\"type\":\"message\",\"timestamp\":\"2024-01-01T00:00:01Z\",\"message\":{\"role\":\"user\",\"content\":\"hello\"}}\n",
            ),
        )
        ...
        assert_eq!(meta.session_id, "abc");
        assert_eq!(meta.cwd, PathBuf::from("/tmp/project"));
        assert_eq!(meta.first_user_message, "hello");
    }

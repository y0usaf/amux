...
mod tests {
    ...
    #[test]
    fn app_config_dir_follows_home_dotdir() {
        let _lock = test_support::env_lock();
        with_env_var_set("HOME", "/tmp/pi-home", || {
            assert_eq!(app_config_dir(), PathBuf::from("/tmp/pi-home/.pi"));
        });
    }

    #[test]
    fn app_dirs_prefer_xdg_state_variables_and_runtime_falls_back_to_state() {
        let _lock = test_support::env_lock();
